  ...
  return useMutation({
    mutationFn: async (data: CustomFormData) => {
      ...
      const obj = {
        ...
        user: JSON.parse(await SecureStore.getItemAsync('userData') || '{}')?.user?.id,
      };
      ...
      console.log('Transformed form data:', formDataToSend);
      return createCandidate(formDataToSend);
    },
    onError: (error) => {
      console.error('Error in useCreateCandidate mutation:', error);
    },
  });
};
